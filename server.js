...
server.registerTool("createRequest", {
    title: "Proof request tool",
    description: "Creates an inklink proof request with the given title category and description",
    inputSchema: {
        title: z.string(),
        category: z.string(),
...
server.registerTool("createSessionId", {
    title: "Request session id tool",
    description: "Creates a session id for an inklink proof request and sets a return url. Used to track a specific user's request",
    inputSchema: {
        id: z.string(),
        url: z.string(),
        api_key: z.string(),
    ...
    const response = await fetch("https://app.inklink.dev/api/request-session-id", {
        method: "POST",
        headers: {
            "x-api-key": api_key,
            "Content-Type": "application/json",
...
server.registerTool("retrieveResults", {
    ...
    try {
        const response = await fetch(`https://app.inklink.dev/api/request-result/${id}`, {
            method: "GET",
            headers: {
                "x-api-key": api_key,
            },
        ...
        return {
            content: [
                {
                    type: "text",
                    text: "Proof was completed with a credibility level of:" +
...
server.registerTool("retrieveSessionResults", {
    ...
    try {
        const response = await fetch(`https://app.inklink.dev/api/request-result/${id}?session_id${sid}`, {
            method: "GET",
            headers: {
                "x-api-key": api_key,
            },
